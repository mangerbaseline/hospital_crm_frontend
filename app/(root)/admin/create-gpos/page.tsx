...
import {
  ...
  getSingleGPO,
  clearSelectedGPO,
} from "@/store/features/gpo/gpoSlice";
import { GPOTable } from "@/components/gpo/GPOTable";
import { GPOModal } from "@/components/gpo/GPOModal";
...
function CreateGPOsPage() {
  ...
  useEffect(() => {
    dispatch(fetchGPOs(queryParams));
  ...
  const handleEdit = async (id: string) => {
    try {
      await dispatch(getSingleGPO(id)).unwrap();
      setIsModalOpen(true);
    } catch (error: any) {
      toast.error(error || "Failed to fetch GPO details");
    }
  };

  const handleDelete = async (id: string) => {
    try {
      await dispatch(deleteGPO(id)).unwrap();
      toast.success("GPO deleted successfully");
      dispatch(fetchGPOs(queryParams));
    } catch (error: any) {
      toast.error(error || "Failed to delete GPO");
    }
  };

  const openAddModal = () => {
    dispatch(clearSelectedGPO());
    setIsModalOpen(true);
  };
  ...
  return (
    <section className="w-full max-w-7xl mx-auto">
      ...
      <GPOModal
        ...
        onSuccess={() => dispatch(fetchGPOs(queryParams))}
        gpo={selectedGPO}
      />
    </section>
  );
